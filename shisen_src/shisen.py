...
def marker_on(x, y):
    ...
    sp_set(4, x, y, sp_code(2, 0, 0, 2), 3)
    sp_set(5, x + 8, y, sp_code(2, 1, 0, 2), 3)
    sp_set(6, x, y + 32, sp_code(2, 0, 1, 2), 3)
...
def pointer_on(x, y):
    x, y = x * TILE_W + ofs_x + 16, y * TILE_H + ofs_y + 16
    sp_set(0, x + 4, y - 3, sp_code(0, 0, 0, 2), 3)
    sp_set(1, x + 4, y + 32 + 3 , sp_code(0, 0, 1, 2), 3)
    sp_set(2, x - 4, y + 16, sp_code(1, 0, 0, 2), 3)
    sp_set(3, x + 8 + 4, y + 16, sp_code(1, 1, 0, 2), 3)
...
def draw_cg(cg, line_color = 0):
    i, x1, y1  = 0, 0, 0
    while i < len(CG[cg]):
        p = CG[cg][i]
        if type(p) is str:
            if p == 'mv':
                x1, y1 = CG[cg][i + 1], CG[cg][i + 2]
                i += 2
            elif p == 'pa':
                paint(CG[cg][i + 1], CG[cg][i + 2], CG[cg][i + 3])
                i += 3
        else:
            if x1 is not None:
                line(x1, y1, CG[cg][i], CG[cg][i + 1], line_color)
                x1 = None
            else:
                line_to(CG[cg][i], CG[cg][i + 1], line_color)
            i += 1
        i += 1
...
def main():
    ...
    sp_color(1, rgb(31, 0, 31), 2)
    sp_color(2, rgb(31, 16, 8), 2)
    sp_color(3, rgb(1, 1, 1), 2)
    sp_disp(1)
    sp_on(0, 7)
    with x68k.Super():
        x68k.iocs(x68k.i.B_WPOKE, a1=0xe82500, d1=0x12e4) # mouse > sprite    
...
PAT = (("3333333333333333322222222222222332222222222222230322222222222230"
        "0032222222222300000322222222300000003222222300000000032222300000"
        "00000032230000000000000330000000" + "0" * 16 * 6),
       ...
       ("3333333333000033311111111300003131111111130000313111111113000031"
        "3113333333000033" + "3113000000000000" * 10 + "3333000000000000"))

CG = (('mv', 391, 241, 391, 259, 393, 262, 399, 250, 399, 246, 394, 244,
       391, 241, 'mv', 74, 177, 73, 177, 71, 183, 77, 215, 77, 216, 85, 203,
       ...
       138, 390, 121, 400, 115, 'pa', 66, 179, 0x07c0, 'pa', 222, 163,
